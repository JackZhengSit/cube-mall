...
public class RestTemplateTest {
    ...
    public void bulkBlog(){
        JSONArray jsonArray = JSON.parseArray("[{\"id\":1, \"title\":\"帮日本核废水“出招”？知名男艺人迷之操作让人看呆了..\", \"content\":\"帮日本核废水“出招”？知名男艺人迷之操作让人看呆了..\", \"comment\":\"娱乐\", \"mobile\":\"13900112233\"}\n" +
                ...
                "]");
        List<IndexQuery> queryList = jsonArray.stream().map(json -> {
            IndexQuery query = new IndexQueryBuilder()
                    .withId(((JSONObject) json).getString("id"))
                    .withSource(((JSONObject) json).toJSONString())
                    .build();
            return query;
        }).collect(Collectors.toList());

        template.bulkIndex(queryList,IndexCoordinates.of("blog_template"));
    }

    @Test
    public void testCriteriaQuery(){
        Criteria criteria = new Criteria("title").is("喜剧");
        Query query=new CriteriaQuery(criteria);
        SearchHits<Blog> hits = template.search(query, Blog.class);
        hits.stream().forEach(h-> System.out.println(h));
    }

    @Test
    public void testNativeSearchQuery() throws IOException {
        NativeSearchQuery query = new NativeSearchQueryBuilder()
                .withQuery(QueryBuilders.multiMatchQuery("看电影", "title", "content"))
                .withFilter(QueryBuilders.boolQuery()
                        .should(QueryBuilders.termsQuery("title", "电影"))
                        .must(QueryBuilders.termsQuery("content", "喜剧"))
                )
                ...
                .addAggregation(AggregationBuilders.terms("mobile_group").field("mobile"))
                .addAggregation(AggregationBuilders.count("mobile").field("mobile"))
...
//            System.out.println(highlightFields);
            ...
            String content = hit.getHighlightField("content").get(0);
        ...
        ParsedStringTerms mobile_group = aggregations.get("mobile_group");
        ParsedValueCount mobile = aggregations.get("mobile");

        mobile_group.getBuckets().forEach(bucket -> {
